...
class CBLDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        filePath = os.path.join(self.img_dir,filename) #self.img_dir + "/" + filename
    ...
    def check(self, label):
        if (
            ...
            return True
class CBLPRD_CE(CBLDataLoader):
    def __init__(self, annoFile, imgSize, lpr_max_len, PreprocFun=None, shuffle: bool = False):
        super().__init__(annoFile, imgSize, lpr_max_len, PreprocFun, shuffle)
        return
    def __getitem__(self, index):
        filename, LicencePlate, class_LP = self.anno_csv.iloc[index, :]
        filePath =os.path.join(self.img_dir, filename) 
        
        img_int = Image.open(filePath)
        img_tensor = self.PreprocFun(img_int)
        len_LP=len(LicencePlate)
        LicencePlate =  LicencePlate.ljust(self.lpr_max_len, '-')
        LP_label =torch.tensor([CHARS_DICT[c] for c in LicencePlate]) 
...
class CBLPRD_CTC(CBLPRD_CE):
    def __getitem__(self, index):
        img_tensor, LP_label, len_LP, class_LP = super().__getitem__(index)
        return img_tensor, LP_label, torch.tensor(len_LP), torch.tensor(class_LP)
    pass


class CBLPRD_CE_1Layer_LP(CBLPRD_CE):
    def __init__(self, annoFile, imgSize, lpr_max_len, PreprocFun=None, shuffle: bool = False):
        super().__init__(annoFile, imgSize, lpr_max_len, PreprocFun, shuffle)
        # delete 2 layer LPs
        self.anno_csv = self.anno_csv[~self.anno_csv.iloc[:, 2].isin([2, 4])]
        return    
    pass

class CBLPRD_CTC_1Layer_LP(CBLPRD_CTC,CBLPRD_CE_1Layer_LP):
    def __init__(self, annoFile, imgSize, lpr_max_len, PreprocFun=None, shuffle: bool = False):
        CBLPRD_CE_1Layer_LP.__init__(self,annoFile, imgSize, lpr_max_len, PreprocFun, shuffle)
    pass
...
def __test_module():
    dataset = CBLPRD_CTC_1Layer_LP("CBLdata/CBLPRD-330k_v1/train.txt", [32, 256], 8)
    one_data = dataset[10]
    # dataset:list=>iter
